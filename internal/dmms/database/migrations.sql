...
-- Kanban tasks (daily work tracking)
CREATE TABLE IF NOT EXISTS dmms_kanban_tasks (
  id             TEXT PRIMARY KEY,
  deliverable_id TEXT NOT NULL REFERENCES dmms_deliverables(id) ON DELETE CASCADE,
  project_id     TEXT NOT NULL REFERENCES dmms_projects(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_ktask_deliverable ON dmms_kanban_tasks(deliverable_id);
CREATE INDEX IF NOT EXISTS idx_ktask_project     ON dmms_kanban_tasks(project_id);
CREATE INDEX IF NOT EXISTS idx_ktask_assigned    ON dmms_kanban_tasks(assigned_to);

-- Kanban task comments
CREATE TABLE IF NOT EXISTS dmms_kanban_comments (
  id         TEXT PRIMARY KEY,
  task_id    TEXT NOT NULL REFERENCES dmms_kanban_tasks(id) ON DELETE CASCADE,
  author_id  TEXT NOT NULL REFERENCES dmms_users(id),
  body       TEXT NOT NULL,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP
);
CREATE INDEX IF NOT EXISTS idx_kcomment_task ON dmms_kanban_comments(task_id);
