...
function withEffectiveDue(data) {
    if (!data || typeof data !== "object")
        return data;
    if (Array.isArray(data))
        return data.map(withEffectiveDue);
    const obj = data;
    ...
    if (Array.isArray(obj.items)) {
        return { ...obj, items: obj.items.map(applyToItem) };
    }
    if (Array.isArray(obj.data)) {
        return { ...obj, data: obj.data.map(applyToItem) };
    }
...
// ─── PROJECTS ─────────────────────────────────────────────────────────────────
server.tool("dmms_list_projects", "List projects. Filterable by status. Returns {items, total, limit, offset}.", {
    status: z
...
// ─── DELIVERABLES ─────────────────────────────────────────────────────────────
server.tool("dmms_deliverable_tree", "Get the full deliverable tree for a project (recursive parent/child structure)", { project_id: z.string().uuid() }, async ({ project_id }) => {
...
server.tool("dmms_my_deliverables", "List deliverables assigned to the current user (contributor), ordered by due_date ascending", {}, async () => {
...
// ─── TASKS (checklist) ────────────────────────────────────────────────────────
server.tool("dmms_list_tasks", "List checklist tasks for a deliverable", { deliverable_id: z.string().uuid() }, async ({ deliverable_id }) => {
    const data = await api("GET", `/api/dmms/deliverables/${deliverable_id}/tasks`);
    return fmt(data);
});
server.tool("dmms_create_task", "Create a checklist task for a deliverable (PM/Admin only)", {
...
server.tool("dmms_update_task", "Update a checklist task (PM/Admin only)", {
    deliverable_id: z.string().uuid(),
...
server.tool("dmms_delete_task", "Delete a checklist task (PM/Admin only)", {
    deliverable_id: z.string().uuid(),
    task_id: z.string().uuid(),
...
server.tool("dmms_list_kanban", "List all kanban tasks with optional filters. Returns {items, total}. Each task includes effective_due_date (task due_date or deliverable_due_date fallback).", {
    ...
    if (params.deliverable_id)
        qs.set("deliverable_id", params.deliverable_id);
    if (params.status)
        qs.set("status", params.status);
    if (params.assignee_id)
        qs.set("assigned_to", params.assignee_id);
    if (params.hide_archived)
        qs.set("hide_archived", "true");
    if (params.from_date)
...
server.tool("dmms_my_kanban", "List kanban tasks for the current user (assigned, member, or deliverable owner). Returns {items, total}. Each task includes effective_due_date.", {
    project_id: z.string().uuid().optional(),
    deliverable_id: z.string().uuid().optional(),
    status: z.enum(["backlog", "todo", "in_progress", "review", "done"]).optional(),
    hide_archived: z.boolean().optional().describe("Exclude archived tasks"),
    ...
    if (params.offset != null)
        qs.set("offset", String(params.offset));
    const data = await api("GET", `/api/dmms/kanban/mine${qs.toString() ? `?${qs}` : ""}`);
    return fmtKanban(data);
});
server.tool("dmms_create_kanban_task", "Create a new kanban task. project_id and deliverable_id are required.", {
    title: z.string(),
    description: z.string().optional(),
    project_id: z.string().uuid(),
    deliverable_id: z.string().uuid(),
...
server.tool("dmms_update_kanban_task", "Update a kanban task (move status, reassign, set due date, etc.)", {
    id: z.string().uuid(),
    title: z.string().optional(),
    description: z.string().optional(),
...
// ─── MARKETPLACE ──────────────────────────────────────────────────────────────
server.tool("dmms_marketplace", "Browse open deliverables available for bidding. visibility=all includes private ones (PM/Admin only).", {
    visibility: z
        .enum(["public", "all"])
        .optional()
...
// ─── ADMIN ────────────────────────────────────────────────────────────────────
server.tool("dmms_list_users", "List all users (Admin only)", {
    limit: z.number().int().min(1).max(200).optional(),
    offset: z.number().int().min(0).optional(),
}, async (params) => {
    const qs = new URLSearchParams();
    ...
    if (params.offset != null)
        qs.set("offset", String(params.offset));
    const data = await api("GET", `/api/dmms/admin/users${qs.toString() ? `?${qs}` : ""}`);
    return fmt(data);
});
